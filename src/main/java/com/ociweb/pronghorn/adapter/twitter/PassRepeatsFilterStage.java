...
public class PassRepeatsFilterStage<T extends MessageSchema> extends PronghornStage {
    ...
    //TODO: Different stage but oposite: need to know all users we have ever followed and only pass those we have not seen before
    //      required for follow of new users from general feed to avoid duplicates
    ...
    public void startup() {
        ...
        if (storage.exists()) {
            try {
                ...
                filters = (BloomFilter[]) oist.readObject();
    ...
    private void buildNewFilters() {
        int i = instances;
        filters = new BloomFilter[i];
        while (--i>=0) {
            filters[i] = new BloomFilter(maximumItems, maximumFailure);
        }
    }
